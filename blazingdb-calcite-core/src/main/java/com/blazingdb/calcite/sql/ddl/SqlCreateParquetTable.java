 ...
 */
public class SqlCreateParquetTable extends SqlCreate implements SqlExecutableStatement {
	private final SqlIdentifier name;
	private final SqlNode folder;
	private final SqlNode wildcard;

	private static final SqlOperator OPERATOR = new SqlSpecialOperator("CREATE PARQUET TABLE", SqlKind.OTHER);

	/** Creates a SqlCreateTable. */
	SqlCreateParquetTable(SqlParserPos pos, SqlIdentifier name, SqlNode folder, SqlNode wildcard) {
		super(OPERATOR, pos, false, false);
		this.name = Preconditions.checkNotNull(name);
		this.folder = Preconditions.checkNotNull(folder);
		this.wildcard = Preconditions.checkNotNull(wildcard);
	}

	public List<SqlNode> getOperandList() {
		SqlNode arg1 = name;
		SqlNode arg2 = folder;
		SqlNode arg3 = wildcard;

		return ImmutableNullableList.of(arg1, arg2, arg3);
	}

	@Override
	public void unparse(SqlWriter writer, int leftPrec, int rightPrec) {
		writer.keyword("CREATE");
		writer.keyword("PARQUET");
		writer.keyword("TABLE");
		name.unparse(writer, leftPrec, rightPrec);
		Frame frame = writer.startList("(", ")");
		folder.unparse(writer, leftPrec, rightPrec);
		writer.sep(",", true);
		wildcard.unparse(writer, leftPrec, rightPrec);
	...
	static class MutableArrayTable extends AbstractModifiableTable implements Wrapper {
		 ...
		 *
		 * @param name
		 *            Name of table within its schema
		 * @param protoStoredRowType
		 *            Prototype of row type of stored columns (all columns except virtual columns)
		 * @param protoRowType
		 *            Prototype of row type (all columns)
		 * @param initializerExpressionFactory
