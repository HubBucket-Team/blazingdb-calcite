...
public class SqlDdlNodes {
	...
	public static SqlCreateSchema createSchema(SqlParserPos pos, boolean replace, boolean ifNotExists,
			SqlIdentifier name) {
		return new SqlCreateSchema(pos, replace, ifNotExists, name);
	}

	/** Creates a CREATE DATABASE. */
	public static SqlCreateDatabase createDatabase(SqlParserPos pos, SqlIdentifier name) {
		return new SqlCreateDatabase(pos, name);
	}

	/** Creates a CREATE FOREIGN SCHEMA. */
	public static SqlCreateForeignSchema createForeignSchema(SqlParserPos pos, boolean replace, boolean ifNotExists,
	...
	/** Creates a CREATE PARQUET TABLE. */
	public static SqlCreateParquetTable createParquetTable(SqlParserPos pos, SqlIdentifier name, SqlNode folder,
			SqlNode wildcard) {
		return new SqlCreateParquetTable(pos, name, folder, wildcard);
	}

	/** Creates a CREATE VIEW. */
	public static SqlCreateView createView(SqlParserPos pos, boolean replace, SqlIdentifier name,
			SqlNodeList columnList, SqlNode query) {
		return new SqlCreateView(pos, replace, name, columnList, query);
	}

	/** Creates a CREATE MATERIALIZED VIEW. */
	public static SqlCreateMaterializedView createMaterializedView(SqlParserPos pos, boolean replace,
	...
	public static SqlDropDatabase dropDatabase(SqlParserPos pos, boolean ifExists, SqlIdentifier name) {
		return new SqlDropDatabase(pos, ifExists, name);
	}
	...
	public static SqlNode column(SqlParserPos pos, SqlIdentifier name, SqlDataTypeSpec dataType, SqlNode expression,
	...
	static SqlNode renameColumns(SqlNodeList columnList, SqlNode query) {
		...
		final SqlCall from = SqlStdOperatorTable.AS.createCall(p,
				ImmutableList.<SqlNode>builder().add(query).add(new SqlIdentifier("_", p)).addAll(columnList).build());
	...
	protected static void populate(SqlIdentifier name, SqlNode query, CalcitePrepare.Context context) {
		...
		// again.)
		final FrameworkConfig config = Frameworks.newConfigBuilder().defaultSchema(context.getRootSchema().plus())
				.build();
